export default async function handler(req, res) {
  ...
  const TARGET = 'https://farside.co.uk/sol/';
  ...
  try {
    ...
    for (const m of rows) {
      ...
      // Détecter header SOL ETF (noms à confirmer selon Farside)
      if (cells.some(c => c.length >= 3 && c.length <= 6 && /^[A-Z]+$/.test(c)) && cells[0] !== 'Date') {
        headers = cells;
        continue;
      }
      const f = cells[0];
    ...
    return res.status(200).json({
      ...
      top1:     topFlows[0] || { name: '—', val: 0 },
      top2:     topFlows[1] || { name: '—', val: 0 },
